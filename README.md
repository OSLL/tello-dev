...
* `follow_marker_with_coords` -- следование за маркером при помощи опредлении координат маркера относительно дрона. Видео с дрона записывается в `drone_stream.avi` файл. *Требуется откалиброванная камеры* (***Пока что данные камеры зашиты в коде***)
...
* `camera_calibration` -- калибровка камеры. ***Ещё не встроено***
