    ...
    if last_state == State.EMPTY:
        ...
        if ids is not None:
            ...
            if is_find:
                ...
                state = get_state(center, marker_size, marker_center)
                actions.append(state)
                last_state = state
        if ids is None or not is_find:
            if time.process_time() - last_time > TIMEOUT:
                print("Send END")
                actions.append((State.END, None))
                last_state = (State.END, None)
    cv2.putText(frame, f"Last state is {last_state}", (0, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    frame = cv2.rectangle(frame, (center[0] - X_RANGE, center[1] - Y_RANGE), (center[0] + X_RANGE, center[1] + Y_RANGE), (0, 255, 0), 2)
    video_writer.write(cv2.resize(frame, FRAME_SIZE))
    return frame
...
def drone_instructions(tello):
    ...
    while True:
        if len(actions) == 0:
            if time.process_time() - last_exec_time > 0.5:
                last_state = State.EMPTY
            continue
        last_exec_time = time.process_time()
        state, movements = actions.pop(0)
        if state == State.END:
            break
